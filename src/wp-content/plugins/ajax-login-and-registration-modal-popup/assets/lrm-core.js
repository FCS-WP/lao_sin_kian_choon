...
//   console.log(LRM.home_url_arr.host , window.location.host);
  ...
  function lrm_init() {
    ...
    function login_selected(event, event_orig) {
      ...
      if (!$formModal.length) {
        ...
        if (
          el &&
          ($(el).hasClass("lrm-redirect") ||
            $(el).parent().hasClass("lrm-redirect"))
        ) {
      ...
      setTimeout(function () {
        if ($(window).width() > 600) {
          $formModal
            .find(".lrm-signin-section input[data-autofocus]")
            .trigger("focus");
        }
      }, 100);
