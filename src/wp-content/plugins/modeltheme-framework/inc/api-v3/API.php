...
DEFINE('MODELTHEME_APIV3_VERSION', '3.2');
...
function modeltheme_theme_license_v3_panel() {
 ...
 ?>
    ...
    <div class="modeltheme-wrap">
            ...
            <form>
                ...
                <?php if(modelthemeav3_is_license_activated()){ ?>
                    ...
                    <?php if(modelthemeav3_get_active_license_type() == 'market'){ ?>
                        ...
                        <p><input type="password" class="widefat" readonly id="purchase_key" name="purchase_key" required value="<?php echo esc_attr($purchase_key); ?>"></p>
                    <?php } ?>
                    ...
                    <?php if(modelthemeav3_get_active_license_type() == 'elements'){ ?>
                        ...
                        <p><input type="password" class="widefat" readonly id="token" name="token" required value="<?php echo esc_attr($purchase_key); ?>"></p>
                    <?php } ?>
        ...
        <?php do_action('modeltheme_after_activation_panel'); ?>
    </div>
...
function modeltheme_add_custom_js_to_admin_head() {
    $screen = get_current_screen();
        // Check if the current screen matches the desired view
    if ($screen && $screen->id === 'toplevel_page_modeltheme-license') {
        ?>
        <script type="text/javascript">
            jQuery(document).ready(function() {
                ...
                jQuery('#modeltheme-activator-v3-form form').on('keypress', function (event) {
...
add_action('admin_head', 'modeltheme_add_custom_js_to_admin_head');
...
add_action('modeltheme_after_activation_panel', 'modeltheme_system_requirements_panel_box', 5);
...
add_action('modeltheme_after_activation_panel', 'modeltheme_apiv3_version', 10);
function modeltheme_apiv3_version(){ ?>
    <div class="modeltheme-sys-version">
        <p><small><i>ModelTheme License Manager (v<?php echo esc_html(MODELTHEME_APIV3_VERSION); ?>)</i></small></p>
    </div><?php
}
...
function modeltheme_get_system_configs()
{
    $settings = [
        ...
        array(
            'title' => esc_html__('PHP Max Execution Time', 'modeltheme'),
